...
@Route(path = RoutePath.TOOLS_CALIBRATION_J1_3DP_LEVELING_BED_AUXILIARY)
public class LevelingBedAuxiliaryCalibrationActivity extends BaseActivity {
    ...
    public void gotoLevelingBedAuxiliaryCalibration() {
        addFragment(R.id.fragment_container, LevelingBedAuxiliaryCalibrationFragment.newInstance());
    }

    public void gotoRestoringMachine() {
        if (isGuide) {
            ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setGuideLevelingBed(true);
