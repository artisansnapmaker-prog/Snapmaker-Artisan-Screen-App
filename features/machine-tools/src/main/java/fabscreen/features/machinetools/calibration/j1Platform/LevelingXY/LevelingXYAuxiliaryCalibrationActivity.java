...
@Route(path = RoutePath.TOOLS_CALIBRATION_J1_3DP_LEVELING_XY_AUXILIARY)
public class LevelingXYAuxiliaryCalibrationActivity extends BaseActivity {
    ...
    public void gotoLevelingXYAuxiliaryCalibration() {
        addFragment(R.id.fragment_container, LevelingXYAuxiliaryCalibrationFragment.newInstance());
    }

    public void gotoRestoringMachine() {
        if (isGuide) {
            ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setGuideLevelingXY(true);
            gotoCalibrationSuccess();
        } else {
