...
public class SetOriginPagerPresenter implements DefaultLifecycleObserver {
    @BindView(R2.id.vp_widget_set_origin_pager)
    ViewPager mVpPager;
    ...
    public SetOriginPagerPresenter(Context context) {
    ...
    private Observable<SSTPPacketContent.GcodeResponse> runBoundaryWith(int index, int speed) {
        if (index == 5) {
            String cmd = String.format(Locale.US, "G0 X0 Y0 F%d", speed);
            ...
            String cmd = String.format(Locale.US, "G0 X%.2f Y%.2f F%d", point[0], point[1], speed);
