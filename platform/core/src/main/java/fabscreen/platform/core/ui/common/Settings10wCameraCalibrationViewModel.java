...
public class Settings10wCameraCalibrationViewModel extends BaseViewModel {
    ...
    public Settings10wCameraCalibrationViewModel() {
        super();

        switch (ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getMachineModel()) {
            case Constants.MACHINE_TYPE_A150:
                cameraZ = 150;
                d = 80;
                mTakePhotoPoint = new Point(155, 82);
                break;
            case Constants.MACHINE_TYPE_A250:
                cameraZ = 230;
                d = 100;
                mTakePhotoPoint = new Point(186, 130);
                break;
            case Constants.MACHINE_TYPE_A350:
                cameraZ = 290;
                d = 100;
                mTakePhotoPoint = new Point(232, 178);
                break;
            default:
                mTakePhotoPoint = new Point((int) ServiceContainer.getInstance().getService(IMachine.class).getMachineInfoSubjectHolder().getValue().size.getX() / 2, (int) ServiceContainer.getInstance().getService(IMachine.class).getMachineInfoSubjectHolder().getValue().size.getY() / 2);
                cameraZ = 140;
                d = 80;
        }
    ...
    private void initPosition() {
        ...
        final float bottomZ = ServiceContainer.getInstance().getService(IPreferences.class).getHelper().getLaserPlatformZ();
        mZPos = bottomZ + MockConst.LASER_10W_LASER_FOCUS;
    ...
    private Observable<ResponseStructure> engraveLine(Point point, int index) {
        ...
        return ServiceContainer.getInstance().getService(IAppService.class).getSlaveComputer().sendGcode("M3 P20")
                .flatMap(response -> {
                    Vector vector = new Vector();
    ...
    private void detectingRectangle() {
        Vector vector = new Vector();
        vector.setX(mTakePhotoPoint.x);
        vector.setY(mTakePhotoPoint.y);
        vector.setZ(cameraZ);
        ServiceContainer.getInstance().getService(IMachine.class).getMachineController().gotoAbsolutePosition(vector)
    ...
    private void process(Bitmap image) {
        ...
        int headerId = (int) (Math.random() * Integer.MAX_VALUE);
        Logger.d("1.14.0 Fix BUG headerId: " + headerId);
        // TODO : Master judgment
//        ServiceContainer.getInstance().getService(IMachine.class).getLaserController().getLaserToolhead().setHeaderOnlineSyncId(headerId)
//                .observeOn(AndroidSchedulers.mainThread())
//                .as(bindToLifecycle())
//                .subscribe(success -> {
//                    if (success) {
//                        ServiceContainer.getInstance().getService(IPreferences.class).getHelper().setHeaderOnlineSyncID(headerId);
//                    } else {
//                        Logger.d("Set HeaderId failed.");
//                    }
//                }, LogHelper::log);
    }
